...
public:
    ...
    // Push
    void push(const ElementType number);
    void push_front(const ElementType number);
...
template <typename ElementType>
void TypedArray<ElementType>::push(const ElementType number) {
    set(size(), number);
}

template <typename ElementType>
void TypedArray<ElementType>::push_front(const ElementType number) {
    while (origin == 0) {
        extend_buffer();
    }
    origin--;
...
TypedArray<ElementType> TypedArray<ElementType>::concat(const TypedArray& other){
    ...
    for (int i = 0; i < size(); i++) {
        temp.push(safe_get(i));
    }
    for (int j = 0; j < other.size(); j++) {
        temp.push(other.safe_get(j));
    }
...
TypedArray<ElementType>& TypedArray<ElementType>::reverse() {
    for (int i = 0; i < size()/2; i++) {
        ElementType temp = safe_get(size()-1-i);
        set(size()-1-i, safe_get(i));
        set(i, temp);
    }
    return *this;
...
TypedArray<ElementType> TypedArray<ElementType>::operator+(const TypedArray& other) {
    TypedArray<ElementType> add = concat(other);
    return add;
}
